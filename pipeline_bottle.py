...
BOTTLE_MODEL_OUTPUT_DIRECTORY = "output/bottle/model"
BOTTLE_DESIGN_OUTPUT_ROOT_DIRECTORY = "output/bottle/design"
BOTTLE_RENDER_OUTPUT_ROOT_DIRECTORY = "output/bottle/render"

BOTTLE_GENERATE_SCRIPT_PATH = "generate_bottle_models.py"
...
def ensure_bottle_models_are_generated(requested_instance_count):
    already_generated_blend_files = glob.glob(os.path.join(BOTTLE_MODEL_OUTPUT_DIRECTORY, "*.blend"))
    if os.path.isdir(BOTTLE_MODEL_OUTPUT_DIRECTORY) and len(already_generated_blend_files) > 0:
        print(f"Bottle models already exist in {BOTTLE_MODEL_OUTPUT_DIRECTORY}, skipping generation.")
        return True
    patch_constant_in_script(BOTTLE_GENERATE_SCRIPT_PATH, "OUTPUT_DIRECTORY", BOTTLE_MODEL_OUTPUT_DIRECTORY)
    return run_blender_script_with_arguments(
        BOTTLE_GENERATE_SCRIPT_PATH, [str(requested_instance_count)])
...
def process_single_design_through_skin_and_render(design_image_path):
    ...
    design_output_directory = os.path.join(BOTTLE_DESIGN_OUTPUT_ROOT_DIRECTORY, design_category, design_name)
    render_output_directory = os.path.join(BOTTLE_RENDER_OUTPUT_ROOT_DIRECTORY, design_category, design_name)

    print(f"\n=== bottle / {design_category}/{design_name} ===")

    patch_constant_in_script(BOTTLE_SKIN_SCRIPT_PATH, "INPUT_DIRECTORY", BOTTLE_MODEL_OUTPUT_DIRECTORY)
    patch_constant_in_script(BOTTLE_SKIN_SCRIPT_PATH, "OUTPUT_DIRECTORY", design_output_directory)
    skin_succeeded = run_blender_script_with_arguments(
        BOTTLE_SKIN_SCRIPT_PATH, [os.path.abspath(design_image_path)])
    if not skin_succeeded:
        print(f"Skin application failed for design {design_category}/{design_name}.")
        return False

    patch_constant_in_script(BOTTLE_RENDER_SCRIPT_PATH, "INPUT_DIRECTORY", design_output_directory)
    patch_constant_in_script(BOTTLE_RENDER_SCRIPT_PATH, "OUTPUT_RENDER_DIRECTORY", render_output_directory)
    render_succeeded = run_blender_script_with_arguments(BOTTLE_RENDER_SCRIPT_PATH, [])
...
def run_full_bottle_pipeline():
    ...
    print(f"Generating {requested_instance_count} bottle instances.")

    generation_succeeded = ensure_bottle_models_are_generated(requested_instance_count)
    if not generation_succeeded:
        print("Bottle model generation failed, aborting pipeline.")
        return
