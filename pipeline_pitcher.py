...
PITCHER_MODEL_OUTPUT_DIRECTORY = "output/pitcher/model"
PITCHER_DESIGN_OUTPUT_ROOT_DIRECTORY = "output/pitcher/design"
PITCHER_RENDER_OUTPUT_ROOT_DIRECTORY = "output/pitcher/render"

PITCHER_GENERATE_SCRIPT_PATH = "generate_pitcher_models.py"
PITCHER_SKIN_SCRIPT_PATH = "apply_skin_to_pitcher_models.py"
PITCHER_RENDER_SCRIPT_PATH = "render_textured_pitchers_to_png.py"
...
def ensure_pitcher_models_are_generated(requested_instance_count):
    already_generated_blend_files = glob.glob(os.path.join(PITCHER_MODEL_OUTPUT_DIRECTORY, "*.blend"))
    if os.path.isdir(PITCHER_MODEL_OUTPUT_DIRECTORY) and len(already_generated_blend_files) >= requested_instance_count:
        print(f"Pitcher models already exist in {PITCHER_MODEL_OUTPUT_DIRECTORY}, skipping generation.")
        return True
    patch_constant_in_script(PITCHER_GENERATE_SCRIPT_PATH, "OUTPUT_DIRECTORY", PITCHER_MODEL_OUTPUT_DIRECTORY)
    return run_blender_script_with_arguments(
        PITCHER_GENERATE_SCRIPT_PATH, [str(requested_instance_count)])
...
def process_single_design_through_skin_and_render(design_image_path):
    ...
    design_output_directory = os.path.join(PITCHER_DESIGN_OUTPUT_ROOT_DIRECTORY, design_category, design_name)
    render_output_directory = os.path.join(PITCHER_RENDER_OUTPUT_ROOT_DIRECTORY, design_category, design_name)

    print(f"\n=== pitcher / {design_category}/{design_name} ===")

    patch_constant_in_script(PITCHER_SKIN_SCRIPT_PATH, "INPUT_DIRECTORY", PITCHER_MODEL_OUTPUT_DIRECTORY)
    patch_constant_in_script(PITCHER_SKIN_SCRIPT_PATH, "OUTPUT_DIRECTORY", design_output_directory)
    skin_succeeded = run_blender_script_with_arguments(
        PITCHER_SKIN_SCRIPT_PATH, [os.path.abspath(design_image_path)])
    if not skin_succeeded:
        print(f"Skin application failed for design {design_category}/{design_name}.")
        return False

    patch_constant_in_script(PITCHER_RENDER_SCRIPT_PATH, "INPUT_DIRECTORY", design_output_directory)
    patch_constant_in_script(PITCHER_RENDER_SCRIPT_PATH, "OUTPUT_RENDER_DIRECTORY", render_output_directory)
    render_succeeded = run_blender_script_with_arguments(PITCHER_RENDER_SCRIPT_PATH, [])
...
def run_full_pitcher_pipeline():
    ...
    print(f"Generating {requested_instance_count} pitcher instances.")

    generation_succeeded = ensure_pitcher_models_are_generated(requested_instance_count)
    if not generation_succeeded:
        print("Pitcher model generation failed, aborting pipeline.")
        return
